...
func NewInformer(client dynamic.ResourceInterface, fields [][]string, transform cache.TransformFunc, gvk schema.GroupVersionKind, db sqlStore.DBClient, shouldEncrypt bool, namespaced bool, watchable bool) (*Informer, error) {
	watchFunc := func(options metav1.ListOptions) (watch.Interface, error) {
		return client.Watch(context.Background(), options)
	}
	if !watchable {
		watchFunc = func(options metav1.ListOptions) (watch.Interface, error) {
			return newSyntheticWatcher().watch(client, options, 5*time.Second)
		}
	}
